...
def init_db():
    ...
    conn.executescript('''
        ...
        CREATE TABLE chats (
            id INTEGER PRIMARY KEY,
            type TEXT NOT NULL, -- 'channel' or 'private'
            name TEXT,
            description TEXT,
            created_by INTEGER,
...
def index():
    ...
    update_seen(user['id'])
    conn = get_db()
    # Fetch user's chats with last message & unread count
    chats = conn.execute('''
        SELECT c.*, 
...
def login():
    if request.method == 'POST':
        ...
        if row:
            ...
            update_seen(row['id'])
            return redirect('/')
        return render_template('index.html', error='Неверный логин или пароль', login=True)
...
def register():
    if request.method == 'POST':
        ...
        try:
            ...
            return render_template('index.html', error='Имя уже занято', register=True)
...
def api_create_chat():
    ...
    if ctype == 'private' and target_uid:
        # Check existing private chat between two
