...
def init_db():
    conn = get_db()
    # Используем IF NOT EXISTS, чтобы не стирать базу при каждом перезапуске Render
    conn.executescript('''
...
def login():
    if request.method == 'POST':
        ...
        if row:
            ...
            update_seen(row['id'])
            return redirect('/')
        return render_template('index.html', error='Неверный логин или пароль', login=True, user=None)
    return render_template('index.html', login=True, user=None)
...
def register():
    if request.method == 'POST':
        ...
        try:
            ...
            return render_template('index.html', error='Имя уже занято', register=True, user=None)
    return render_template('index.html', register=True, user=None)
